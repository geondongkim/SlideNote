...
| **PDF 내보내기** | 주석 + 노트를 합쳐 PDF 저장 (검색 가능 Searchable PDF) |
| **PPTX 폰트 자동 탐색** | PPTX에 사용된 폰트를 시스템 탐색 → 없으면 Google Fonts에서 자동 다운로드 |
...
pywin32 (subprocess)   — PPTX → PNG (Windows, Python subprocess로 COM STA 충돌 해결)
LibreOffice headless   — PPTX → PNG (Linux/Mac 폴백)
